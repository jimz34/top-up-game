...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "NeonTop — Instant Game Top-Up for Mobile Legends, Free Fire & More" },
      { name: "description", content: "Top up diamonds, UC, VP and more in seconds. Trusted by gamers, secure payments, 24/7 WhatsApp support." },
      { property: "og:title", content: "NeonTop — Instant Game Top-Up" },
      { property: "og:description", content: "Instant diamonds for your favorite mobile games." },
    ],
  }),
  component: HomePage,
});
...
function HomePage() {
  ...
  const filtered = games.filter((g) => g.name.toLowerCase().includes(q.toLowerCase()) || (g.category ?? "").toLowerCase().includes(q.toLowerCase()));
  ...
  return (
    <div>
      ...
      <section className="relative overflow-hidden">
        <img src={heroBg} alt="" aria-hidden className="absolute inset-0 h-full w-full object-cover opacity-40" width={1920} height={1088} />
        ...
        <div className="relative container mx-auto px-4 pt-16 pb-20 md:pt-24 md:pb-28">
          <div className="max-w-3xl">
            ...
            <p className="mt-5 text-base md:text-lg text-muted-foreground max-w-2xl">
              Mobile Legends, Free Fire, PUBG, Valorant, Genshin Impact &amp; more — secure payments, automatic processing, real-time order updates.
            </p>
            ...
            <div className="mt-8 glass-strong rounded-2xl p-2 max-w-xl flex items-center gap-2 neon-ring">
              ...
              />
              <Button size="sm" className="bg-[var(--gradient-primary)] text-primary-foreground">Search</Button>
            </div>

            <div className="mt-8 flex flex-wrap gap-3 text-sm text-muted-foreground">
              <span className="inline-flex items-center gap-1.5"><Zap className="h-4 w-4 text-[var(--neon)]" /> Instant</span>
              <span className="inline-flex items-center gap-1.5"><ShieldCheck className="h-4 w-4 text-[var(--neon)]" /> Secure</span>
              <span className="inline-flex items-center gap-1.5"><Sparkles className="h-4 w-4 text-[var(--neon)]" /> Best price</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-12 md:py-16">
        <div className="flex items-end justify-between mb-6">
          ...
          <Link to="/games" className="text-sm text-muted-foreground hover:text-foreground">View all →</Link>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {filtered.map((g) => (
            <Link key={g.id} to="/games/$slug" params={{ slug: g.slug }}
              className="group glass rounded-2xl overflow-hidden hover-glow">
              <div className="flex">
                <img src={gameImage(g.slug)} alt={g.name} width={120} height={150}
                  loading="lazy" className="h-28 w-24 object-cover" />
                <div className="p-3 flex-1 min-w-0">
                  <p className="font-semibold truncate">{g.name}</p>
                  <p className="text-xs text-muted-foreground truncate">{g.publisher}</p>
                  <span className="inline-block mt-2 text-[10px] uppercase tracking-wider rounded-full bg-secondary/60 px-2 py-0.5">{g.category}</span>
                </div>
              </div>
            </Link>
          ))}
          {filtered.length === 0 && (
            <p className="col-span-full text-muted-foreground text-center py-12">No games match "{q}".</p>
          )}
        </div>
      </section>
