...
function GameDetailPage() {
  ...
  const fetch = useServerFn(getGameWithProducts);
  ...
  if (isLoading) return <div className="container mx-auto px-4 py-20 text-center text-muted-foreground"><Loader2 className="mx-auto h-6 w-6 animate-spin" /></div>;
  ...
  const handleSubmit = async () => {
    if (!user) {
      ...
      router.navigate({ to: "/login", search: { redirect: `/games/${slug}` } });
      return;
    }
  ...
  return (
    <div className="container mx-auto px-4 py-10">
      <Link to="/games" className="text-sm text-muted-foreground hover:text-foreground">← All games</Link>

      <div className="mt-4 glass-strong rounded-3xl p-6 md:p-8 grid md:grid-cols-[260px_1fr] gap-6 items-center neon-ring">
        <img src={gameImage(game.slug)} alt={game.name} width={512} height={640}
        ...
        <div>
          ...
          <p className="mt-4 text-sm text-muted-foreground max-w-prose">
            Enter your account details, pick a package, and complete payment to receive your top-up instantly.
          </p>
        </div>
      </div>
      ...
      <div className="mt-8 grid lg:grid-cols-3 gap-6">
          ...
          <Step n={1} title="Enter Account">
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <Input value={userGameId} onChange={(e) => setUserGameId(e.target.value)} placeholder="e.g. 123456789" />
              </div>
              {game.requires_server_id && (
                <div>
                  <Label>Server ID</Label>
                  <Input value={serverId} onChange={(e) => setServerId(e.target.value)} placeholder="e.g. 2034" />
                </div>
              )}
            </div>
          </Step>
              ...
              {products.map((p) => {
                ...
                return (
                  <button key={p.id} type="button" onClick={() => setProductId(p.id)}
                    className={`relative text-left rounded-xl p-4 transition glass ${active ? "ring-2 ring-[var(--neon)] neon-ring" : "hover-glow"}`}>
                    <p className="font-semibold">{p.name}</p>
                    <p className="mt-1 neon-text text-lg font-bold">{formatIDR(Number(p.price))}</p>
              ...
              {PAYMENT_METHODS.map((m) => {
                ...
                return (
                  <button key={m.id} type="button" onClick={() => setPaymentMethod(m.id)}
                    className={`text-left rounded-xl p-4 glass ${active ? "ring-2 ring-[var(--neon)] neon-ring" : "hover-glow"}`}>
                    <p className="font-semibold">{m.label}</p>
                    <p className="text-xs text-muted-foreground mt-1">
                      {m.id === "wallet" ? "Pay instantly from your wallet" : "Secure payment via Midtrans"}
                    </p>
                  </button>
                );
              })}
        ...
        <aside className="glass-strong rounded-2xl p-6 h-fit sticky top-20">
          ...
          <dl className="mt-4 space-y-2 text-sm">
            ...
            <Row label="Payment" value={PAYMENT_METHODS.find((m) => m.id === paymentMethod)?.label ?? ""} />
          </dl>
          <div className="my-5 h-px bg-border" />
          <div className="flex items-baseline justify-between">
            <span className="text-muted-foreground text-sm">Total</span>
            <span className="text-2xl font-bold neon-text">{selected ? formatIDR(Number(selected.price)) : "—"}</span>
          </div>
  ...
  return (
    <section className="glass-strong rounded-2xl p-6">
      <header className="flex items-center gap-3 mb-4">
        <span className="grid h-8 w-8 place-items-center rounded-full bg-[var(--gradient-primary)] text-primary-foreground font-bold text-sm">{n}</span>
